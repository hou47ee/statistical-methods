...
def chi2_test_of_independence(feat_row, feat_col, alpha=0.05):
    ...
    # delete unnecessary values "-1" for refusing answering
    feat_row = feat_row[feat_col != -1]
    feat_col = feat_col[feat_col != -1]        
    feat_col = feat_col[feat_row != -1]
    feat_row = feat_row[feat_row != -1]
...
def level_to_yes_no_data_preprocessing(data):
...
if __name__ == "__main__":
    ...
    Dental_Health = npha.data.features['Dental_Health']
    Dental_Health = Dental_Health[Dental_Health != 6] # drop the data of Wearing Dentures

    Employment = npha.data.features['Employment']
    Stress_Keeps_Patient_from_Sleeping = npha.data.features['Stress_Keeps_Patient_from_Sleeping']
    Medication_Keeps_Patient_from_Sleeping = npha.data.features['Medication_Keeps_Patient_from_Sleeping']
    Pain_Keeps_Patient_from_Sleeping = npha.data.features['Pain_Keeps_Patient_from_Sleeping']
    Bathroom_Needs_Keeps_Patient_from_Sleeping = npha.data.features['Bathroom_Needs_Keeps_Patient_from_Sleeping']
    Uknown_Keeps_Patient_from_Sleeping = npha.data.features['Uknown_Keeps_Patient_from_Sleeping']
    Trouble_Sleeping = npha.data.features['Trouble_Sleeping']
    Prescription_Sleep_Medication = npha.data.features['Prescription_Sleep_Medication']
    Race = npha.data.features['Race'] 
    Gender = npha.data.features['Gender']
    Number_of_Doctors_Visited = npha.data.targets['Number_of_Doctors_Visited']
    
    spearman_rank_corr(Physical_Health, Mental_Health)


    # Data preprocessoing for Chi-squared test
    Trouble_Sleeping = level_to_yes_no_data_preprocessing(Trouble_Sleeping)
    Prescription_Sleep_Medication = level_to_yes_no_data_preprocessing(Prescription_Sleep_Medication)

    chi2_test_of_independence(Trouble_Sleeping, Number_of_Doctors_Visited)
